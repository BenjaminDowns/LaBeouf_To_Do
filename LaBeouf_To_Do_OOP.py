...
class Do_It:
	...
	def __init__(self):
		...
		###### Controls Buttons ######
		# Add item button #
		add_item_button = ttk.Button(controls, text = "DO WHAT?", command = self.add_new_item)
		add_item_button.grid(column = 0, row = 0, stick = 'nw')
		# Print Button #
		print_button = ttk.Button(controls, text = 'PRINT THEM ALL!', command = self.print_all)
		print_button.grid(column = 1, row = 0)
		# Delete item button #
		delete_item_button = ttk.Button(controls, text = "WHAT DID YOU DO?", command = self.delete_item)
		delete_item_button.grid(column = 3, row = 0, stick = 'nw')

		###### Main list frame #####
		self.default_to_do = ttk.Frame(root, height = 300, width = 400, relief = SUNKEN) # Make the main body frame
		self.default_to_do.pack()

		###### Background image for main list ######
		background_image = PhotoImage(file = 'LaBeouf_image.gif')
		background_label = Label(self.default_to_do, image = background_image)
	...
	def add_new_item(self):
		...
		if 'entry1' not in globals():
			entry1 = ttk.Entry(self.default_to_do, width = 30)
			entry1.pack()
			entry1.insert(0, x)
